...
export default function CuisineTablette() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      const newOrder: KitchenOrder = {
        ...
        serverName: ['Marie', 'Pierre', 'Sophie'][Math.floor(Math.random() * 3)],
        items: [
          {
            id: Date.now().toString(),
            name: ['Steak Frites', 'Saumon Grillé', 'Risotto', 'Burger'][Math.floor(Math.random() * 4)],
  ...
  const updateItemStatus = (orderId: string, itemId: string, newStatus: string) => {
    setOrders(prev => prev.map(order => {
      if (order.id === orderId) {
        ...
        return { ...order, items: updatedItems, status: newOrderStatus as any }
      }
      return order
    }))
  }
    ...
    setOrders(prev => prev.map(order => {
      if (order.id === orderId) {
        ...
        if (newStatus === 'DELIVERED') {
          ...
          console.log(`📢 Commande ${order.orderNumber} prête pour la table ${order.tableNumber}`)
          alert(`Commande ${order.orderNumber} prête! Serveur ${order.serverName} notifié.`)
        }
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'DELIVERED': return <CheckSquare className="h-4 w-4" />
    ...
    <div className="min-h-screen bg-gray-50 p-4">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-4 mb-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold">Cuisine - Tablet de Commandes</h1>
                <p className="text-sm text-gray-600">Gestion des commandes en temps réel</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <div className="text-sm text-gray-600">Commandes actives</div>
                <div className="text-xl font-bold">{orders.filter(o => o.status !== 'DELIVERED').length}</div>
              </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-6 gap-4 mb-4">
          ...
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-purple-600">{stats.averagePreparationTime}</div>
              <div className="text-xs text-gray-600">Temps moyen</div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 text-center">
              <div className="text-2xl font-bold text-orange-600">{stats.ordersPerHour}</div>
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
          <TabsList className="grid w-full grid-cols-4">
            <TabsTrigger value="en-cours" className="flex items-center gap-2">
              ...
              En cours ({getOrdersByStatus('PENDING').length + getOrdersByStatus('PREPARING').length})
            </TabsTrigger>
              ...
              {getOrdersByStatus('PENDING').concat(getOrdersByStatus('PREPARING')).map((order) => (
                <Card key={order.id} className={`border-l-4 ${getPriorityColor(order.priority)}`}>
                  <CardHeader className="pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div>
                          ...
                          <CardDescription>
                            Table {order.tableNumber} • Serveur: {order.serverName}
                          </CardDescription>
                        </div>
                      ...
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => printTicket(order)}>
                    ...
                    <div className="mt-2">
                      <div className="flex justify-between text-sm mb-1">
                        ...
                        <span>{Math.round(calculateOrderProgress(order))}%</span>
                      </div>
                  ...
                  <CardContent className="space-y-3">
                    {order.items.map((item) => (
                      <div key={item.id} className="border rounded-lg p-3">
                        ...
                        <div className="flex items-center gap-4 text-xs text-gray-500 mt-2">
                          {item.startedAt && (
                            <span>Début: {getTimeElapsed(item.startedAt)} min</span>
                          )}
                          {item.estimatedTime && (
                    ...
                    {order.status === 'PREPARING' && calculateOrderProgress(order) === 100 && (
                      <Button 
                  ...
                  <CardContent>
                    ...
                    <div className="mt-3 p-3 bg-green-50 border border-green-200 rounded-lg">
                      <div className="flex items-center gap-2 text-green-800">
                        ...
                        <span className="font-medium">Prête à être servie!</span>
                      </div>
                      <div className="text-sm text-green-700 mt-1">
                        Notifier le serveur {order.serverName} pour la table {order.tableNumber}
                      </div>
                    </div>
                  </CardContent>
                </Card>
                ...
                <Card key={order.id} className="opacity-75">
                  ...
                  <CardContent>
                    <div className="space-y-1">
                      {order.items.map((item) => (
                        <div key={item.id} className="text-sm justify-between flex">
                          <span>{item.quantity}x {item.name}</span>
                        </div>
                      ))}
                    </div>
